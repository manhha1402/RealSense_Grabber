...
// ------------ Convert image to gray scale if necessary -------------------
    ...
    if (debug)
    {
        cv::namedWindow("00 Grayscale",cv::WINDOW_NORMAL);
...
// ------------ Filtering --------------------------------------------------
    if (false)
    {
        ...
        if (debug)
        {
          cv::namedWindow("10 filtering closed",cv::WINDOW_NORMAL);
        ...
        if (debug)
        {
          cv::namedWindow("11 filtering divide",cv::WINDOW_NORMAL);
...
// ------------ Adaptive thresholding --------------------------------------
    ...
    if (debug)
    {
      cv::namedWindow("20 Adaptive thresholding",cv::WINDOW_NORMAL);
...
// ------------ Contour extraction --------------------------------------
    ...
    if (debug)
    {
        ...
        cv::namedWindow("30 Contours",cv::WINDOW_NORMAL);
...
// ------------ Ellipse extraction --------------------------------------
    ...
    if (debug)
    {
        ...
        if(m_use_fast_pi_tag)
        {
            ...
            cv::namedWindow("80 Ellipsevoting",cv::WINDOW_NORMAL);
            cv::namedWindow("90 Ellipsedensity",cv::WINDOW_NORMAL);
            cv::imshow("80 Ellipsevoting", ellipsevoting);
            cv::imshow("90 Ellipsedensity", ellipsedensity_img);
        }
        //Fast Pi Tag -ENd-
        cv::namedWindow("40 Ellipses",cv::WINDOW_NORMAL);
    ...
    for(size_t n = 0; n < points.size() || (!m_use_fast_pi_tag && once); n++){ //Each point is the center of a roi
        ...
        if (debug)
        {
            ...
            cv::namedWindow("50 Lines",cv::WINDOW_NORMAL);
        ...
        if (debug)
        {
            ...
            cv::namedWindow("51 Valid Lines",cv::WINDOW_NORMAL);
       ...
       for (unsigned int i=0; i<final_tag_vec.size(); i++)
        ...
        if (debug)
        {
          cv::namedWindow("52 refined ellipses", cv::WINDOW_NORMAL);
        ...
        if (final_debug)
        {
            ...
            cv::namedWindow("60 Tags",cv::WINDOW_NORMAL);
...
unsigned long FiducialModelPi::GetPose(cv::Mat& image, std::vector<t_pose>& vec_pose)
{
    ...
    for (unsigned int i=0; i<vec_points.size(); i++)
    {
        ...
        reprojection_matrix.convertTo(reprojection_matrix, CV_64FC1); //add
...
unsigned long FiducialModelPi::GetCorrespondences(cv::Mat& image1,cv::Mat& image2,cv::Mat& depth1,cv::Mat& depth2,
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr& keypoint_cloud1,pcl::PointCloud<pcl::PointXYZRGB>::Ptr& keypoint_cloud2,
	pcl::CorrespondencesPtr& correspondences)
{
	keypoint_cloud1.reset(new pcl::PointCloud<pcl::PointXYZRGB>);
	keypoint_cloud2.reset(new pcl::PointCloud<pcl::PointXYZRGB>);
	correspondences.reset(new pcl::Correspondences);
	bool debug = false;
	 vector<pair<cv::Point2f,cv::Point2f> > marker_matches;
	  std::vector<t_points> marker_vector1,marker_vector2;

	    if (GetPoints(image1, marker_vector1) != RET_OK)
	        return RET_FAILED;
	    if (GetPoints(image2, marker_vector2) != RET_OK)
	    	return RET_FAILED;
	 ...
	 if (id1 == id2) {
		 for(int k=0;k<marker_vector1[i].image_points.size();k++)
				 {
			 marker_matches.push_back(std::make_pair(marker_vector1[i].image_points[k], marker_vector2[j].image_points[k]));
			      }
		 	 break;
	         }
	        }
...
{
	pcl::Correspondence it;
	cv::Point2f markers1 = marker_matches[i].first;
	cv::Point2f markers2 = marker_matches[i].second;
	pcl::PointXYZRGB pt1,pt2;
	 	 	int x1 = static_cast<int>(round(markers1.x));
	        int y1 = static_cast<int>(round(markers1.y));
	        int x2 = static_cast<int>(round(markers2.x));
	        int y2 = static_cast<int>(round(markers2.y));
	        //check value of depth =0 or = nan value, removed
	 if(!std::isnan(depth1.at<unsigned short>(y1,x1)) && !std::isnan(depth2.at<unsigned short>(y2,x2))
	 	 && (depth1.at<unsigned short>(y1,x1)!=0) && (depth2.at<unsigned short>(y2,x2) !=0))
	 {

		pt1.z = depth1.at<unsigned short>(y1,x1)/10000.0;
		pt1.x = ((x1-GetCameraMatrix().at<double>(0,2))*pt1.z/GetCameraMatrix().at<double>(0,0));
		pt1.y = ((y1-GetCameraMatrix().at<double>(1,2))*pt1.z/GetCameraMatrix().at<double>(1,1));
		pt1.r = image1.at<cv::Vec3b>(y1,x1)[2];
		pt1.g = image1.at<cv::Vec3b>(y1,x1)[1];
		pt1.b = image1.at<cv::Vec3b>(y1,x1)[0];
		keypoint_cloud1->points.push_back(pt1);

				pt2.z = depth2.at<unsigned short>(y2,x2)/10000.0;
				pt2.x = ((x2-GetCameraMatrix().at<double>(0,2))*pt2.z/GetCameraMatrix().at<double>(0,0));
				pt2.y = ((y2-GetCameraMatrix().at<double>(1,2))*pt2.z/GetCameraMatrix().at<double>(1,1));
				pt2.r = image2.at<cv::Vec3b>(y2,x2)[2];
				pt2.g = image2.at<cv::Vec3b>(y2,x2)[1];
				pt2.b = image2.at<cv::Vec3b>(y2,x2)[0];
		keypoint_cloud2->points.push_back(pt2);
		it.index_query = count;
		it.index_match = count;
		it.distance = std::sqrt(std::pow(pt1.x-pt2.x,2)+std::pow(pt1.y-pt2.y,2)+std::pow(pt1.z-pt2.z,2));
...
keypoint_cloud1->width = (uint32_t)keypoint_cloud1->size();
keypoint_cloud1->height = 1;
keypoint_cloud2->width = (uint32_t)keypoint_cloud2->size();
keypoint_cloud2->height =1;


if(debug)
{
	  vector<cv::KeyPoint> p1,p2;
	  vector<cv::DMatch> matches;
	for(int i=0;i<marker_matches.size();i++)
	{
		p1.push_back(cv::KeyPoint(marker_matches[i].first,1.0f));
		p2.push_back(cv::KeyPoint(marker_matches[i].second,1.0f));
		cv::DMatch it;
		it.queryIdx =i;
		it.trainIdx=i;
		matches.push_back(it);
	}
	cv::Mat drawMatches; //draw image with key points
	    cv::drawMatches(image1,p1, image2, p2, matches, drawMatches,cv::Scalar::all(-1),cv::Scalar::all(-1),
